...
class ContactUsController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($keyword != null) {
            $message = $message->search($keyword);
        }
        if ($status != null) {
            $message = $message->whereStatus($status);
        }
        $message = $message->orderBy($sort_by, $order_by);
        $message = $message->paginate($limit_by);

        return view('backend.contact_us.index', compact('message'));
    }
